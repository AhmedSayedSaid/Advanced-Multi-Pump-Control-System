...
const unsigned char fctsupported[] =
{
	...
	MB_FC_WRITE_MULTIPLE_HOLDING_REGISTERS
};
	...
	xSemaphoreTake(modH->ModBusSphrHandle , portMAX_DELAY); //before processing the message get the semaphore
	 ...
	 switch(modH->u8Buffer[ FUNC ] )
	 {
			...
			case MB_FC_WRITE_MULTIPLE_COILS:
				...
				break;
			case MB_FC_WRITE_MULTIPLE_HOLDING_REGISTERS :
                   ...
                   modbus_t *specificTelegram, osThreadId_t taskToNotify, uint32_t timeout)
{
    ...
    osDelay(timeout);
    // Suspend the task and wait for notification or timeout
   // uint32_t u32NotificationValue = ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(timeout));
